...
import { Search, Plus, Download, Trophy, Target, TrendingUp, Star, Zap, Shield } from 'lucide-react';
import { RESULT_BADGE } from '@/shared/lib/constants';
import { supabase } from '@/lib/supabase';

export function MatchEntries() {
  const { matchEntries, players, matches, addMatchEntry, updateMatchEntry, removeMatchEntry, fetchMatchEntries } = useFootballStore();
  const [modal, setModal] = useState<{ type: 'add' | 'edit' | 'delete' | 'bulk', data?: MatchEntry } | null>(null);
  ...
  // ── Bulk import from player seasons ──────────────────────────
  const handleBulkImport = async () => {
    setBulkLoading(true);
    try {
      const entries: Omit<MatchEntry, 'id'>[] = [];
      for (const player of players) {
        const seasons = (player as any).seasons ?? [];
        for (const season of seasons) {
          for (const month of season.monthlyStats ?? []) {
            for (const week of month.weeklyStats ?? []) {
              const dates: string[] = week.matchDates ?? [];
              if (dates.length === 0) continue;

              // Distribute stats across match dates in this week
              const totalMatches = dates.length;
              let winsLeft = week.win ?? 0;
              let lossesLeft = week.loss ?? 0;
              let goalsLeft = week.goalsScored ?? 0;
              let concededLeft = week.goalsConceded ?? 0;
              const motmWeek = week.motm ?? 0;
              const csWeek = week.cleanSheet ?? 0;
              const htWeek = week.hattricks ?? 0;

              dates.forEach((date, di) => {
                const isLast = di === totalMatches - 1;
                let result: 'win' | 'loss' | 'draw' = 'draw';
                let goals = 0;
                let conceded = 0;

                if (!isLast) {
                  if (winsLeft > 0) { result = 'win'; winsLeft--; }
                  else if (lossesLeft > 0) { result = 'loss'; lossesLeft--; }
                  // Distribute goals roughly
                  goals = result === 'win' ? Math.min(goalsLeft, Math.ceil(goalsLeft / (totalMatches - di))) : 0;
                  conceded = result === 'loss' ? Math.min(concededLeft, Math.ceil(concededLeft / (totalMatches - di))) : 0;
                  goalsLeft -= goals;
                  concededLeft -= conceded;
                } else {
                  // Last match gets the remainder
                  if (winsLeft > 0) result = 'win';
                  else if (lossesLeft > 0) result = 'loss';
                  goals = Math.max(0, goalsLeft);
                  conceded = Math.max(0, concededLeft);
                }

                const hattricks = goals >= 3 && htWeek > 0 ? 1 : 0;
                ...
                entries.push({
                  ...
                  result,
                  hattricks,
                  cleanSheet: conceded === 0 && csWeek > 0 && di === 0,
                  motm: motmWeek > 0 && di === 0,
                  date,
                  notes: `Bulk import — Season ${season.year}, Month ${month.month}, Week ${week.week}`,
                  source: 'bulk',
                } as any);
              });
            }
      ...
      if (entries.length === 0) {
        alert('No match dates found in player seasons. Add season data with dates first!');
      ...
      for (let i = 0; i < entries.length; i += BATCH) {
        const { error } = await supabase.from('match_entries').insert(entries.slice(i, i + BATCH));
        if (error) throw error;
      }

      await fetchMatchEntries();
      alert(`✅ Successfully imported ${entries.length} match entries from player seasons!`);
      setModal(null);
    } catch (err: any) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      {/* Bulk Import Confirm */}
      {modal?.type === 'bulk' && (
        <Modal title="Bulk Import from Seasons" onClose={() => setModal(null)} isOpen>
          <div className="p-4 space-y-4">
            <p className="text-[14px] text-foreground">
              This will read all <strong>match dates</strong> from every player's season data and create individual match entries automatically.
            </p>
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3">
              <p className="text-[12px] text-amber-400">
                ⚠️ Make sure you have added season data with match dates to your players first. Entries will be distributed from weekly stats.
              </p>
            </div>
            <p className="text-[13px] text-muted-foreground">
              Players with seasons: <strong className="text-foreground">{players.filter(p => (p as any).seasons?.length > 0).length}</strong>
            </p>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between mb-6 gap-4">
        ...
        <div className="flex gap-3 items-center flex-wrap">
          ...
          <Button variant="secondary" onClick={() => setModal({ type: 'bulk' })}>
            <Download className="w-4 h-4 mr-1.5" /> Bulk Import
          </Button>
                ...
                {playerStats.length === 0 && (
                  <tr>
                    <td colSpan={11} className="py-12 text-center text-muted-foreground">
                      No match entries yet. Add entries manually or use Bulk Import.
                    </td>
                  </tr>
                )}
              </tbody>
