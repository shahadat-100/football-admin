...
export function MatchImport() {
  ...
  const updateMappedEntry = (index: number, updates: Partial<typeof mappedEntries[0]>) => {
    setMappedEntries(prev => {
      ...
      // Manually edited goals clear the "ambiguous, please check" flag.
      if (updates.goals !== undefined || updates.goalsConceded !== undefined) {
        nw[index].ambiguousScore = false;
      }
    ...
    <div className="flex flex-col gap-6 max-w-5xl mx-auto pb-10 animate-in fade-in">
      ...
      {step === 2 && parsedData && (
        <div className="space-y-6">
          ...
          {(ambiguousCount > 0 || unparsedCount > 0) && (
            <div className="p-4 bg-amber-950/40 border border-amber-500/40 text-amber-200 rounded-lg text-sm space-y-2">
              {ambiguousCount > 0 && (
                <p>⚠️ {ambiguousCount} entr{ambiguousCount === 1 ? 'y has' : 'ies have'} an ambiguous goal split (highlighted below) — please double-check GS/GC.</p>
              )}
              {unparsedCount > 0 && (
                <div>
                  <p>⚠️ {unparsedCount} line{unparsedCount === 1 ? '' : 's'} looked like a result but couldn't be parsed — add manually if needed:</p>
                  <ul className="list-disc pl-5 mt-1 space-y-0.5 font-mono text-[11px] text-amber-300/80">
                    {parsedData.unparsedLines.map((l, i) => <li key={i}>{l}</li>)}
                  </ul>
                </div>
              )}
            </div>
                ...
                >
                  ...
                  <div className="md:col-span-1 flex flex-col justify-center items-end h-full gap-1">
                    <Input
                      type="date"
                      value={entry.date}
                      onChange={e => updateMappedEntry(idx, { date: e.target.value })}
                      className="text-[10px] h-7 px-1 w-full"
                    />
                    <Input
                      type="time"
                      value={entry.time}
                      onChange={e => updateMappedEntry(idx, { time: e.target.value })}
                      className="text-[10px] h-7 px-1 w-full"
                    />
                  </div>
                </div>
