  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use(async (req, res, next) => {
        ...
        // /api/me
        if (req.url === '/api/me') {
          // For local dev simulation, we just return the admin user for now if we want to simulate logged in,
          // OR return 401 if we want to test login screen. Let's return 401 to force login test.
          res.statusCode = 401;
